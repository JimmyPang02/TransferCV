...
def get_parser():
    ...
    # dataset type
    parser.add_argument('--dataset', type=str, default='dg5')
    return parser
...
def load_data(args):
    ...
    modified: dataset input
    '''
    folder_src = os.path.join(args.data_dir, args.src_domain)
    folder_tgt = os.path.join(args.data_dir, args.tgt_domain)
    
    source_loader, n_class = data_loader.load_data(
        folder_src, args.batch_size, dataset="office-home",infinite_data_loader=not args.epoch_based_training, train=True, num_workers=args.num_workers)
    target_train_loader, _ = data_loader.load_data(
        folder_tgt, args.batch_size, dataset="office-home",infinite_data_loader=not args.epoch_based_training, train=True, num_workers=args.num_workers)
    target_test_loader, _ = data_loader.load_data(
        folder_tgt, args.batch_size, dataset="office-home",infinite_data_loader=False, train=False, num_workers=args.num_workers)
...
def train(source_loader, target_train_loader, target_test_loader, model, optimizer, lr_scheduler, args):
    ...
    for e in range(1, args.n_epoch+1):
        ...
        # log
        log.append([train_loss_clf.avg, train_loss_transfer.avg, train_loss_total.avg, test_loss, test_acc])
    ...
    print('Transfer result: {:.4f}'.format(best_acc))
     # 使用列表推导式和.cpu().numpy()确保在转换之前将Tensors移动到CPU
    log_cpu = [[item.cpu().numpy() if torch.is_tensor(item) else item for item in log_row] for log_row in log]
    np_log = np.array(log_cpu, dtype=float)
    np.savetxt(f"{args.transfer_loss}_{args.src_domain}2{args.tgt_domain}_{best_acc}.csv", np_log, delimiter=',', fmt='%.6f')
...
#     if  args.src_domain=="RealWorld" or args.tgt_domain=="RealWorld":
#         print(args.src_domain, args.tgt_domain)
    
    setattr(args, "device", torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
    print(args)
    set_random_seed(args.seed)
    source_loader, target_train_loader, target_test_loader, n_class = load_data(args)
    setattr(args, "n_class", n_class) # 根据类别数，自适应地获取对应的网络结构
