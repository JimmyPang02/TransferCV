...
if __name__ == '__main__':
    ...
    loss_list = alg_loss_dict(args)#根据不同算法，得到不同的loss，如DAN就有分类和迁移loss
    train_loaders, eval_loaders = get_img_dataloader(args)# 根据不同算法，获取不同的dataloader定义
    eval_name_dict = train_valid_target_eval_names(args) 
    algorithm_class = alg.get_algorithm_class(args.algorithm)# 获取算法类
    algorithm = algorithm_class(args).cuda() # 获取算法类
    algorithm.train()
