...
def generate_experiment_cfgs(id):
    ...
    def config_from_vars():
        ...
        # 每interval保存一次checkpoint
        cfg['checkpoint_config'] = dict( 
            by_epoch=False, interval=iters//10, max_keep_ckpts=15,create_symlink=False) 
        # 取消把最新模型链接到latest.pth，服务器环境不支持MMCV库的链接操作，会改成复制
        
        cfg['evaluation'] = dict(interval=iters//10 , metric='mIoU') # 每max_iter/10个iter评估一次
    ...
    # -------------------------------------------------------------------------
    # Set some defaults 这里的参数设置优先级最高，可以覆盖配置文件
    # -------------------------------------------------------------------------
    cfgs = []
    n_gpus = 1
    batch_size = 2 # 4  # 得在这里改bs  # 这里的BS是per GPU的
    iters = 40000  #40000  
    ...
    architecture = None
    workers_per_gpu = 4 # 1  # 得在这里改numworkers
    ...
    # -------------------------------------------------------------------------
    # DGInStyle -> Mit-B5 Backbone, DAFormer-based, shade
    # -------------------------------------------------------------------------
    if id == 60:
        seeds = [0, 1, 2]
        use_dg_dataset = True
        ...
        # DAFormer w SHADE, w/o RCS
        # 使用SHADE  而RCS其实已经在数据集准备阶段使用了
        architecture, backbone, uda, rcs_T, schedule, shade = ('daformer_sepaspp', 'mitb5', 'dacs_fdthings_srconly', None, 'poly10warm', True)

        # 这里是每个随机种子跑一次结果
        for seed in seeds:
            source, target, crop, rcs_min_crop = gta2cs
            inference = 'slide' # Inference with Overlapping Sliding Window
            gpu_model = 'NVIDIAGeForceRTX4090Ti'
            ...
            if shade:
                gpu_model = 'NVIDIAGeForceRTX4090Ti'
            cfg = config_from_vars() # 把前面定义的所有相关变量存进cfg设置变量
            cfgs.append(cfg)
    # -------------------------------------------------------------------------
    # DGInStyle -> Mit-B5 Backbone, DAFormer-based, default
    # -------------------------------------------------------------------------
    elif id==59:
        seeds = [0, 1, 2]
        use_dg_dataset = True
        ...
        for seed in seeds:
            ...
            cfgs.append(cfg)        
    # -------------------------------------------------------------------------
    # DGInStyle -> Mit-B5 Backbone, DAFormer-based, ReVT Aug(Pixmix&Bilateral)
    # -------------------------------------------------------------------------
    elif id==1:
        seeds = [0, 1, 2]
        use_dg_dataset = True
        gta2cs = ('gengtaReVTAug', 'cityscapes', '512x512', 0.5)
    ...
    # -------------------------------------------------------------------------
    # DAFormer -> Mit-B5 Backbone 
    # -------------------------------------------------------------------------
    elif id==2:
        seeds = [0, 1, 2]
        use_dg_dataset = True
        gta2cs = ('gta', 'cityscapes', '512x512', 0.5)
        batch_size = 2  # 这里的BS是per GPU的
        
        # DAFormer w/o SHADE, w/ RCS
        architecture, backbone, uda, rcs_T, schedule, shade = ('daformer_sepaspp', 'mitb5', 'dacs_fdthings_srconly', 0.01, 'poly10warm', False)
